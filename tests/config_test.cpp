...
filesystems: 
    ...
    third:
        ...
        REQUIRE(config.validFilesystems("c", std::vector<string>{}) == std::vector<string>{"second","third"} );
        // see if userdefault and acl works
        REQUIRE(config.validFilesystems("a", std::vector<string>{}) == std::vector<string>{"second","first","third"});
        // see if ACL works, default does not override ACL
        REQUIRE(config.validFilesystems("b", std::vector<string>{"gb"}) == std::vector<string>{"second","third"});
        // global, groupdefault, others
        REQUIRE(config.validFilesystems("d", std::vector<string>{"gb"}) == std::vector<string>{"first","second","third"});
        // userdefault first, group second, multiple groups
        REQUIRE(config.validFilesystems("a", std::vector<string>{"gc","gb"}) == std::vector<string>{"second","first","third"});
        // user first, others, no denied, not only for first user, multiple groups
        REQUIRE(config.validFilesystems("y", std::vector<string>{"ga","gc"}) == std::vector<string>{"third","second"});
        // admin user check, sees all filesystems
        REQUIRE(config.validFilesystems("e", std::vector<string>{}) == std::vector<string>{"first","second", "third"});
    }
